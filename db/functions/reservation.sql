...
create function get_reservations_for_date(p_date date) returns setof reservation_info as $$
...
create function get_pending_reservations() returns setof pending_reservation_info as $$
	with reservation_tables as (
		select
			rt.reservation_id,
			jsonb_agg(rt.floorplan_table_id order by rt.floorplan_table_id) as table_ids,
...
create function get_pending_count() returns pending_count_result as $$
...
create function get_pending_dates() returns setof date as $$
...
) returns reservation_create_result as $$
declare
	v_status text;
	v_id int;
begin
...
create function update_reservation_status(p_id int, p_status text) returns reservation_update_result as $$
	update reservation
	set status = p_status
	where id = p_id
...
create function update_reservation(
	...
	p_notes text default null
) returns reservation_id_result as $$
	update reservation
	set
...
create function assign_reservation_tables(p_reservation_id int, p_table_ids int[]) returns void as $$
begin
...
do $$
declare
	v_result reservation_create_result;
	v_update_result reservation_update_result;
	v_info reservation_info;
	v_list reservation_info[];
	v_dates date[];
	v_count pending_count_result;
	res_id int;
	fp_id int;
	t1_id int;
	t2_id int;
begin
	...
	-- get_reservations_for_date returns empty set when no reservations
	select array_agg(r) into v_list from get_reservations_for_date('2026-01-20') r;
	assert v_list is null, 'should return empty set';

	-- get_pending_count returns 0 when no reservations
	v_count := get_pending_count();
	assert v_count.count = 0, 'pending count should be 0';
	...
	select * into v_info from get_reservations_for_date('2026-01-20') limit 1;
	assert v_info.table_names->>0 = '1', 'table name should match';
	...
	select * into v_info from get_reservations_for_date('2026-01-20') limit 1;
	assert jsonb_array_length(v_info.table_ids) = 2, 'should have 2 tables';
	...
	select array_agg(r) into v_list from get_reservations_for_date('2026-01-20') r;
	assert array_length(v_list, 1) = 2, 'should have 2 reservations';

	-- get_pending_dates returns dates with pending reservations
	select array_agg(d) into v_dates from get_pending_dates() d;
	assert array_length(v_dates, 1) = 1, 'should have 1 pending date';
