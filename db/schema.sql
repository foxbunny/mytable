...
CREATE FUNCTION public.assign_reservation_tables(p_reservation_id integer, p_table_ids integer[]) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
begin
...
CREATE FUNCTION public.block_table(p_table_id integer, p_notes text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.check_table_availability(p_date date, p_time time without time zone, p_duration integer, p_party_size integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
	with slot_times as (
		select
...
CREATE FUNCTION public.cleanup_expired_blocks() RETURNS jsonb
    LANGUAGE sql
    AS $$
	with deleted as (
...
CREATE FUNCTION public.delete_floorplan(p_id integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.delete_floorplan_table(p_id integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.echo(p_message text) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.get_blocked_tables() RETURNS jsonb
    LANGUAGE sql
    AS $$
	select coalesce(jsonb_agg(jsonb_build_object(
		'id', b.id,
...
CREATE FUNCTION public.get_customer_notification(p_token text) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
declare
	v_session record;
...
CREATE FUNCTION public.get_floorplan_tables(p_floorplan_id integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
	select coalesce(jsonb_agg(jsonb_build_object(
		'id', id,
...
CREATE FUNCTION public.get_floorplans() RETURNS jsonb
    LANGUAGE sql
    AS $$
	select coalesce(jsonb_agg(jsonb_build_object(
		'id', id,
...
CREATE FUNCTION public.get_new_pending_reservations(p_since timestamp with time zone DEFAULT NULL::timestamp with time zone) RETURNS jsonb
    LANGUAGE sql
    AS $$
	select coalesce(jsonb_agg(jsonb_build_object(
		'id', r.id,
...
CREATE FUNCTION public.get_pending_count() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.get_pending_dates() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.get_pending_reservations() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.get_reservations_for_date(p_date date) RETURNS jsonb
    LANGUAGE sql
    AS $$
	with reservation_tables as (
		select
...
CREATE FUNCTION public.get_restaurant() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.get_table_status_for_date(p_date date) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.get_tables_for_slot(p_date date, p_time time without time zone, p_duration integer, p_exclude_reservation_id integer DEFAULT NULL::integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
	with slot_times as (
		select
...
CREATE FUNCTION public.is_authenticated() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.is_restaurant_configured() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.is_setup() RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.mark_notification_delivered(p_token text) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
declare
	v_session record;
...
CREATE FUNCTION public.resolve_reservation(p_id integer, p_status text, p_admin_message text DEFAULT NULL::text, p_table_ids integer[] DEFAULT NULL::integer[]) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
declare
	v_token text;
...
CREATE FUNCTION public.save_floorplan(p_name text, p_image_path text, p_image_width integer, p_image_height integer, p_sort_order integer DEFAULT 0) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.save_floorplan_table(p_floorplan_id integer, p_x_pct numeric, p_y_pct numeric, p_name text DEFAULT NULL::text, p_capacity integer DEFAULT 4, p_notes text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql
    AS $_$
declare
	v_name text;
...
CREATE FUNCTION public.save_restaurant(p_name text, p_address text DEFAULT NULL::text, p_phone text DEFAULT NULL::text, p_working_hours jsonb DEFAULT '{}'::jsonb) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.setup_admin(p_username text, p_password text) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
begin
...
CREATE FUNCTION public.unblock_table(p_table_id integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.update_floorplan(p_id integer, p_name text, p_sort_order integer DEFAULT NULL::integer) RETURNS jsonb
    LANGUAGE sql
    AS $$
	update floorplan
	set name = p_name,
...
CREATE FUNCTION public.update_floorplan_table(p_id integer, p_name text DEFAULT NULL::text, p_capacity integer DEFAULT NULL::integer, p_notes text DEFAULT NULL::text, p_x_pct numeric DEFAULT NULL::numeric, p_y_pct numeric DEFAULT NULL::numeric) RETURNS jsonb
    LANGUAGE sql
    AS $$
...
CREATE FUNCTION public.update_reservation_status(p_id integer, p_status text) RETURNS jsonb
    LANGUAGE sql
    AS $$
	update reservation
	set status = p_status
